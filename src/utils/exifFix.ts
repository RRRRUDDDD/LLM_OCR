  ...
  try {
    ...
    while (offset < view.byteLength - 2) {
      ...
      if (marker === 0xffe1) {
        ...
        const read16 = (offsetValue: number) => view.getUint16(offsetValue, !bigEndian);
 ...
 */
export async function fixExifOrientation(blob: Blob | File, signal?: AbortSignal): Promise<Blob | File> {
  throwIfAborted(signal);
  const orientation = await readExifOrientation(blob, signal);
